...
double a_penny_doubled_everyday(int, double total_amount = 0.01);//----WRITE THE FUNCTION PARAMETER LIST WITHIN THE PARENTHESES
...
double a_penny_doubled_everyday(int n, double total_amount) {//----WRITE THE FUNCTION PARAMETER LIST WITHIN THE PARENTHESES
    ...
    if(n <=1) return total_amount; // n = 3
    
    // we are pre-decrimenting because that n value reps which day number we are on. The instructions are listed poorly, 
    // but in reality whats happening is, they meant say the day number we are on. so for day 3, it will be doubled n-1 times. 
    return a_penny_doubled_everyday(--n, total_amount*2); // n = 2, 0.02 // n = 1, 0.04
    // return total_amount; 
